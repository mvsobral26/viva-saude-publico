...
exports.beneficiariosMock = exports.usuariosMock = void 0;
...
const ESPECIALIDADE_POR_CATEGORIA = {
    cardio: 'Cardiologia',
...
const PROCEDIMENTOS_POR_ESPECIALIDADE = {
...
const CONSULTAS_POR_ESPECIALIDADE = {
...
const EXAMES_POR_ESPECIALIDADE = {
    Cardiologia: 'cardio',
...
function buildDeclaracao(id) {
    ...
    const autoavaliacaoSaude = maybe(id * 41, 0.12) ? 'Ruim' : maybe(id * 43, 0.28) ? 'Regular' : 'Boa';
    return {
        hipertensao,
        diabetes,
        tabagismo,
...
function buildCondicao(_, declaracao) {
    const condicoes = [];
    if (declaracao.doencasPreexistentes?.diabetes)
        condicoes.push('Diabetes');
    if (declaracao.doencasPreexistentes?.hipertensao)
        condicoes.push('Hipertensão');
    if (declaracao.doencasPreexistentes?.obesidade)
        condicoes.push('Obesidade');
    if (declaracao.doencasPreexistentes?.doencaRenalCronica)
        condicoes.push('Doença renal crônica');
    if (declaracao.doencasPreexistentes?.asma || declaracao.doencasPreexistentes?.dpoc)
        condicoes.push('Condição respiratória crônica');
    if (declaracao.doencasPreexistentes?.historicoOncologico)
        condicoes.push('Histórico oncológico em seguimento');
    if (declaracao.doencasPreexistentes?.doencaNeurologica)
        condicoes.push('Condição neurológica crônica');
    if (declaracao.doencasPreexistentes?.herniaDeDisco ||
...
function inferExamCategory(condicao, declaracao) {
    const c = condicao.toLowerCase();
    if (c.includes('oncol') || declaracao.doencasPreexistentes?.historicoOncologico)
        return 'onco';
    if (c.includes('neurol') || declaracao.doencasPreexistentes?.doencaNeurologica)
        return 'neuro';
    if (c.includes('renal') || declaracao.doencasPreexistentes?.doencaRenalCronica)
        return 'renal';
    if (c.includes('respirat') || declaracao.doencasPreexistentes?.asma || declaracao.doencasPreexistentes?.dpoc)
        return 'respiratorio';
    if (c.includes('osteo') || c.includes('lomb') || c.includes('artr') || c.includes('artro'))
        return 'ortho';
    if (c.includes('diabetes') || c.includes('obesidade') || declaracao.diabetes || declaracao.doencasPreexistentes?.obesidade)
        return 'metabolico';
    if (c.includes('hipertens'))
        return 'cardio';
    if (declaracao.doencasPreexistentes?.cardiopatia || declaracao.doencasPreexistentes?.insuficienciaCardiaca)
        return 'cardio';
    return 'geral';
}
function buildMedicamentos(id, declaracao) {
    const meds = [];
    if (declaracao.doencasPreexistentes?.hipertensao)
        meds.push({ nome: 'Losartana', classe: 'Anti-hipertensivo' });
    if (declaracao.doencasPreexistentes?.diabetes)
        meds.push({ nome: 'Metformina', classe: 'Antidiabético' });
    if (declaracao.doencasPreexistentes?.obesidade && maybe(id * 263, 0.55))
        meds.push({ nome: 'Orlistate', classe: 'Controle metabólico' });
    if (maybe(id * 269, 0.40))
        meds.push({ nome: 'Sinvastatina', classe: 'Estatina' });
    if (maybe(id * 271, 0.18))
        meds.push({ nome: 'Omeprazol', classe: 'Gastroprotetor' });
    if (maybe(id * 277, 0.15))
        meds.push({ nome: 'AAS', classe: 'Antiagregante' });
    if (maybe(id * 281, 0.08))
        meds.push({ nome: 'Furosemida', classe: 'Diurético' });
    return meds.slice(0, 6);
}
function ev(tipo, diasAtras, nome, categoria, status = 'Realizado', especialidadeAssistencial) {
    return { tipo, diasAtras, nome, categoria, status, especialidadeAssistencial };
}
function linhaAssistencialBase(category, id) {
    if (category === 'geral') {
        return id % 2 === 0 ? 'Gastroenterologia' : 'Clínica geral';
    }
    return ESPECIALIDADE_POR_CATEGORIA[category];
}
function consultaParaLinha(linha, seed) {
    return pick(CONSULTAS_POR_ESPECIALIDADE[linha], seed);
}
function procedimentoParaLinha(linha, seed) {
    return pick(PROCEDIMENTOS_POR_ESPECIALIDADE[linha], seed);
}
function exameParaLinha(linha, fallback, seed) {
...
function examName(category, seed) {
    return pick(EXAMES[category], seed);
}
function buildPerfil(id) {
    const value = hash(id * 307);
    if (value < 0.18)
        return 'REDUNDANCIA_EXAME';
    if (value < 0.36)
        return 'PA_EVITAVEL';
    if (value < 0.56)
        return 'CONSULTA_BAIXA_RESOL';
    if (value < 0.74)
        return 'REPETICAO_ASSISTENCIAL';
    return 'ESTAVEL';
}
function buildEventos(id, perfil, category) {
    ...
    if (perfil === 'REPETICAO_ASSISTENCIAL') {
        ...
        const linhaSecundaria = linhaPrincipal === 'Cardiologia' ? 'Endocrinologia'
    ...
    if (maybe(id * 617, 0.35))
        events.push(ev('Consulta', randInt(id * 619, 4, 12), pick(CONSULTAS, id + 36), 'Ambulatorial'));
    if (maybe(id * 631, 0.20))
        events.push(ev('Procedimento', randInt(id * 641, 54, 80), pick(PROCEDIMENTOS, id + 5), 'Procedimento'));
    return events;
}
function calcularScore(id, declaracao, eventos, medicamentos) {
    let score = 8;
    if (declaracao.doencasPreexistentes?.diabetes)
        score += 16;
    if (declaracao.doencasPreexistentes?.hipertensao)
    ...
    if (declaracao.doencasPreexistentes?.obesidade)
        score += 12;
    if (!declaracao.atividadeFisicaRegular)
        score += 10;
    if (!declaracao.acompanhamentoRegular)
        score += 10;
    if (declaracao.internacaoRecente)
        score += 14;
    if (declaracao.tabagismo)
        score += 8;
    if (eventos.filter((e) => e.tipo === 'Pronto atendimento').length >= 2)
        score += 12;
    if (eventos.filter((e) => e.tipo === 'Exame').length >= 3)
        score += 8;
    if (medicamentos.length >= 4)
        score += 8;
    if (declaracao.percepcaoSaude?.autoavaliacaoSaude === 'Ruim')
        score += 10;
    return Math.min(100, Math.round(score));
}
function calcularRisco(score) {
    if (score >= 72)
        return 'Alto';
    if (score >= 42)
        return 'Médio';
...
function buildStatus(score, id) {
    if (score >= 90)
        return 'Atenção imediata';
    if (score >= 75)
        return 'Monitoramento intensivo';
    if (score >= 55)
        return pick(['Monitoramento pendente', 'Acompanhamento pendente', 'Sem acompanhamento'], id);
    return pick(['Em dia', 'Monitoramento pendente', 'Acompanhamento pendente'], id);
}
function buildAlerta(score, condicao) {
    if (score >= 85)
        return 'Alto custo potencial no curto prazo';
    if (condicao.includes('Diabetes'))
        return 'Diabetes sem acompanhamento regular';
    if (condicao.includes('Hipertensão'))
        return 'Hipertensão sem acompanhamento regular';
    if (condicao.includes('Obesidade'))
        return 'Sedentarismo declarado';
    return 'Perfil estável, manter prevenção';
}
function calcularTransferenciasRisco(totalMedio, totalBaixo) {
    return {
        doMedioParaAlto: Math.round(totalMedio * 0.15),
        doBaixoParaAlto: Math.round(totalBaixo * 0.19),
    };
}
function contarEventosPorTipo(eventos, tipo) {
    return eventos.filter((evento) => evento.tipo === tipo);
}
function existeGrupoExameRedundante(eventos) {
...
const EXAMES_SEGUIMENTO_ATIVO = new Set([
    'Hemoglobina glicada',
    'Glicemia em jejum',
    'Perfil lipídico',
    'Insulina basal',
...
function isEventoSeguimentoAtivoParaSemAcompanhamento(evento) {
    if (evento.status === 'Agendado')
        return true;
    if (evento.tipo === 'Consulta' && evento.diasAtras <= 45)
        return true;
    if (evento.tipo === 'Procedimento' && evento.diasAtras <= 45)
        return true;
...
function normalizarEventoSemAcompanhamento(evento, beneficiarioId, indiceEvento) {
...
function normalizarBeneficiarioSemAcompanhamento(beneficiario) {
...
function normalizarSemAcompanhamento(beneficiarios) {
    return beneficiarios.map(normalizarBeneficiarioSemAcompanhamento);
}
function calcularIndiceCriticidade(beneficiario) {
    ...
    if (declaracao.doencasPreexistentes?.historicoOncologico)
        indice += 12;
    if (declaracao.doencasPreexistentes?.dpoc)
        indice += 12;
    if (!declaracao.acompanhamentoRegular)
        indice += 16;
    if (declaracao.internacaoRecente)
    ...
    if (declaracao.percepcaoSaude?.autoavaliacaoSaude === 'Ruim')
        indice += 12;
    if (declaracao.percepcaoSaude?.autoavaliacaoSaude === 'Regular')
        indice += 6;
    if (medicamentos.length >= 4)
        indice += 10;
    if (medicamentos.length >= 2)
        indice += 4;
    if (condicao.includes('Doença renal crônica'))
        indice += 8;
    if (condicao.includes('Histórico oncológico'))
        indice += 8;
    ...
    if (janela30 >= 3)
        indice += 10;
    if (existeGrupoExameRedundante(eventos))
        indice += 14;
    return indice;
}
function recalibrarScoreParaRiscoAlto(beneficiario, origem) {
...
function recalibrarRisco(beneficiarios) {
    ...
    return beneficiarios.map((beneficiario) => {
        const devePromover = !altosOriginais.includes(beneficiario.id) &&
            (promovidosMedio.has(beneficiario.id) || promovidosBaixo.has(beneficiario.id));
        if (!devePromover)
            return beneficiario;
        const origem = promovidosMedio.has(beneficiario.id) ? 'Médio' : 'Baixo';
...
const beneficiariosBase = Array.from({ length: 250 }, (_, idx) => {
    const id = idx + 1;
...
exports.beneficiariosMock = normalizarSemAcompanhamento(recalibrarRisco(beneficiariosBase));
