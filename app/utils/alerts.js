...
exports.gerarAlertas = gerarAlertas;
function temEventoRecente(beneficiario, termos, limiteDias = 30) {
...
function adicionar(alertas, tipo, mensagem) {
    const existe = alertas.some((alerta) => alerta.mensagem === mensagem);
    if (!existe)
        alertas.push({ tipo, mensagem });
}
function gerarAlertas(beneficiario) {
    const alertas = [];
    ...
    const cardiopatia = declaracao?.doencasPreexistentes?.cardiopatia === true || declaracao?.doencasPreexistentes?.insuficienciaCardiaca === true;
    ...
    const saudeMental = declaracao?.doencasPreexistentes?.depressao === true || declaracao?.doencasPreexistentes?.ansiedade === true;
    const polifarmacia = declaracao?.tratamentosContinuos?.polifarmacia === true || quantidadeMedicamentos >= 5;
    const tratamentoOncologicoAtual = declaracao?.tratamentosContinuos?.tratamentoOncologicoAtual === true;
    ...
    const prontoAtendimentoRecorrente = declaracao?.internacoesExames?.prontoAtendimentoRecorrente === true;
    ...
    const teveInternacaoRecente = internacaoUltimos30Dias || temEventoRecente(beneficiario, ['internação', 'internacao'], 60);
    const teveProntoAtendimentoRecente = prontoAtendimentoRecorrente || temEventoRecente(beneficiario, ['pronto atendimento', 'pronto-atendimento', 'pa', 'emergência', 'emergencia'], 45);
    if (custoPotencial30d >= 20000)
        adicionar(alertas, 'Crítico', 'Alto custo potencial no curto prazo');
    else if (custoPotencial30d >= 10000)
    ...
    if (atividadeFisicaRegular === false)
        adicionar(alertas, 'Médio', 'Sedentarismo declarado');
