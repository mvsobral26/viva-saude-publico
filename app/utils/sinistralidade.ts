...
import { identificarOportunidadeEficiencia, type TipoOportunidade } from './efficiency';

export type ConfiguracaoSinistralidade = {
  receitaMensalCarteira: number;
  metaSinistralidade: number; // 0.70 = 70%
  faixaAtencao: number; // 0.85 = 85%
};
...
function classificarFaixa(indice: number, meta: number, faixaAtencao: number): FaixaSinistralidade {
...
function ordenarOportunidades(
  ...
  if (b.custoEstimado !== a.custoEstimado) return b.custoEstimado - a.custoEstimado;
...
export function calcularResumoSinistralidade(
  ...
  const porRiscoBase: Record<Risco, { quantidade: number; custoTotal: number }> = {
  ...
  > = {
    ...
    'Exame com possível redundância': { quantidade: 0, custoEstimado: 0, potencialReducao: 0 },
    'Consulta com baixa resolutividade': { quantidade: 0, custoEstimado: 0, potencialReducao: 0 },
    'Repetição assistencial': { quantidade: 0, custoEstimado: 0, potencialReducao: 0 },
  };

  beneficiarios.forEach((beneficiario) => {
    porRiscoBase[beneficiario.risco].quantidade += 1;
    porRiscoBase[beneficiario.risco].custoTotal += beneficiario.custoPotencial30d;

    const oportunidade = identificarOportunidadeEficiencia(beneficiario);

    if (oportunidade) {
      const custoRealAssociado = beneficiario.custoPotencial30d;
      const fatorReducao = getFatorReducao(oportunidade);

      oportunidadesBase[oportunidade].quantidade += 1;
      oportunidadesBase[oportunidade].custoEstimado += custoRealAssociado;
      oportunidadesBase[oportunidade].potencialReducao += custoRealAssociado * fatorReducao;
    }
  });
  ...
  const sinistralidadeAjustada = limitarPercentual(
    Math.max(custoTotalProjetado - economiaPotencial, 0) / config.receitaMensalCarteira
  );
  ...
  const porRisco = (
    ...
    .map(([risco, valores]) => ({
      ...
      participacaoCusto:
        custoTotalProjetado > 0 ? limitarPercentual(valores.custoTotal / custoTotalProjetado) : 0,
    }))
    .sort((a, b) => b.custoTotal - a.custoTotal);

  const porOportunidade = (
    Object.entries(oportunidadesBase) as Array<
      [TipoOportunidade, { quantidade: number; custoEstimado: number; potencialReducao: number }]
    >
  )
    .map(([tipo, valores]) => ({
      tipo,
  ...
  const principalDriver =
    porOportunidade.find((item) => item.custoEstimado > 0)?.tipo ?? 'Sem driver relevante';
